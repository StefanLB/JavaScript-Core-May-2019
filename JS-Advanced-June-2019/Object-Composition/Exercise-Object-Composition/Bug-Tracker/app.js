function solve() {
    return (() => {
        ...
        const report = function (author, description, reproducible, severity) {
            reports[counter] = { ID: counter++, author, description, reproducible, severity, status: 'Open' };
            draw();
        };
        ...
        const sort = function (method) {
            reports.sort((a, b) => method === 'author' ? a[method].localeCompare(b[method]) : a[method] - b[method]);
            draw();
        };
